...
public class FindFoodActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        user = getIntent().getStringExtra("phone");
    ...
    private void fetchFoodData() {
        if(!checkConnection(FindFoodActivity.this)) return;
        fetchData = new AsyncTask<Void, Void, Void>() {
            @Override
            protected void onPreExecute() {
                foodData.clear();
                progressDialog = new ProgressDialog(FindFoodActivity.this);
                progressDialog.show();
            ...
            protected Void doInBackground(Void... voids) {
                ref = database.getReference("/leftover-food-donation/FoodDetails/" + user + "/");
                if(!isOnline(FindFoodActivity.this)) return null;
            ...
            protected void onPostExecute(Void aVoid) {
                ...
                if (foodData.size() != 0) {
                    FoodListAdapter adapter = new FoodListAdapter(FindFoodActivity.this, foodData, user,1);
